...
func main() {
	...
	x := symbolic.NewSymbolicVariable("x", symbolic.ExpressionType{Kind: symbolic.IntType})
	y := symbolic.NewSymbolicVariable("y", symbolic.ExpressionType{Kind: symbolic.IntType})
	...
	*/
	a := symbolic.NewSymbolicVariable("a", symbolic.ExpressionType{Kind: symbolic.IntType})
	b := symbolic.NewSymbolicVariable("b", symbolic.ExpressionType{Kind: symbolic.IntType})

	gt := symbolic.NewBinaryOperation(a, b, symbolic.GT)

	maxExpr := symbolic.NewTernaryOperation(gt, a, b, symbolic.IFELSE)
	fmt.Printf("Сложное (max) выражение: %s\n", maxExpr.String())

	z3MaxExpr, err := translator.TranslateExpression(maxExpr)

	if err != nil {
		log.Fatalf("Ошибка трансляции сложного (max) выражения: %v", err)
	}

	fmt.Printf("Сложное (max) Z3 выражение создано: %T\n", z3MaxExpr)
}
